...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc = document[:min(len(document), truncated_len)]

        # [prefix] [masked_content] [suffix]
        masked_length = random.randint(int((1 / 8) * truncated_len), int((3 / 8) * truncated_len))
        # prefix
        prefix_length = random.randint(1, truncated_len - masked_length - 1)
        prefix = truncated_doc[:prefix_length]
        # content
        masked_content = truncated_doc[prefix_length: prefix_length + masked_length]
        suffix = truncated_doc[prefix_length + masked_length:]
        # rearrange
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        while(len(masked_string) < self.block_size):
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        for _, example in zip(range(4), name_dataset):
            x, y = example
            print('x:', '%s' % ''.join([name_dataset.itos[int(c)] for c in x]).encode('utf-8').decode('ascii', errors='ignore'))
